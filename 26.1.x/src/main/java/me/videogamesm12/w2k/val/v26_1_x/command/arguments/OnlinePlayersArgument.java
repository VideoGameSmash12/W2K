...
public class OnlinePlayersArgument implements ArgumentType<String>
{
    ...
    public static OnlinePlayersArgument both()
    {
        return new OnlinePlayersArgument(true);
    }

    public static OnlinePlayersArgument names()
    {
        return new OnlinePlayersArgument(false);
    }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder)
    {
        if (includeUuids)
        {
            return SharedSuggestionProvider.suggest(getOnlinePlayers().stream()
                    .map(entry -> List.of(entry.w2k$profile().w2k$name(), entry.w2k$profile().w2k$uuid().toString()))
                    .flatMap(Collection::stream), builder);
        }

        return SharedSuggestionProvider.suggest(getOnlinePlayers().stream().map(entry -> entry.w2k$profile().w2k$name()), builder);
    }
