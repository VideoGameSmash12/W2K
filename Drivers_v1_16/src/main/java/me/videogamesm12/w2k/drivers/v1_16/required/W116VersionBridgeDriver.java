...
import me.videogamesm12.w2k.drivers.v1_16.mixin.accessor.ClientWorldAccessor;
import me.videogamesm12.w2k.drivers.v1_16.mixin.accessor.DHAccessor;
import me.videogamesm12.w2k.drivers.v1_16.mixin.accessor.IGHAccessor;
...
@WDriverMetadata(identifier = "16_version_bridge")
public class W116VersionBridgeDriver implements WVersionBridgeDriver
{
    @Override
    ...
    public void displayMessage(Component text)
    {
        MinecraftClient.getInstance().inGameHud.getChatHud().addMessage(
                Text.Serializer.fromJson(ComponentUtils.serializeComponent(text).toString()));
    }
    ...
    public List<InventoryEntry> getOpenInventory()
    {
        ...
        return handler.slots.stream().filter(Objects::nonNull).map(slot ->
        {
            ...
            return new InventoryEntry(Text.Serializer.toJsonTree(entry.getName()),
                    ...
                    entry.getTag() != null ? entry.getTag().toString() : null);
    ...
    public List<TileEntry> getNearbyTileEntities()
    {
        ...
        return MinecraftClient.getInstance().world.tickingBlockEntities.stream().map(tile ->
                new TileEntry(Objects.requireNonNull(Registry.BLOCK.getId(tile.getCachedState().getBlock())).toString(),
                        tile.getPos().getX(),
                        tile.getPos().getY(),
                        tile.getPos().getZ(),
                        tile.toInitialChunkDataNbt().toString())).collect(Collectors.toList());
    }
}
