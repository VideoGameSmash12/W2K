...
public abstract class AbstractOverlayRenderer<T extends Overlay>
{
    ...
    {
        context.drawText(MinecraftClient.getInstance().textRenderer, text, x, y, ColorHelper.whiteWithAlpha(255), shadow);
    }

    public void centeredText(DrawContext context, Text text, int x, int y)
    {
        context.drawCenteredTextWithShadow(MinecraftClient.getInstance().textRenderer, text, x, y, ColorHelper.whiteWithAlpha(255));
    }
    ...
    {
        return ColorHelper.getArgb(color.getAlpha(), color.getRed(), color.getGreen(), color.getBlue());
    }
}
